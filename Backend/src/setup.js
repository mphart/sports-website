        ...
        //     if(!divisions.includes(team.division)){
            ...
            for(let i = 0; i < numMatches; i++){
                ...
                for(let j = 0; j < teams.length / 2; j++){
                    let match = {
                home: homeFirst ? teams[j].team_id : teams[j+teams.length / 2].team_id,
                away: homeFirst ? teams[j+teams.length / 2].team_id : teams[j].team_id,
            }
            homeFirst = !homeFirst;
...
async function executePendingMatches(){
    ...
    matches.forEach((match)=>{
        ...
        db.query("UPDATE matches SET match_status=? WHERE match_id=?", 
            [match.match_status, match.match_id]);
    })
    console.log(matches);
}
